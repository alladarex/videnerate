...
def run_distributed_search(
    *,
    ...
    """Fetch from enabled sources and return merged (media_type, url, thumb_bytes)."""
    merged: list[tuple[str, str, bytes]] = []
    seen_urls: set[str] = set()

    def add_items(media_type: str, items: list[tuple[str, bytes]]) -> None:
        for url, thumb in items:
            if url in seen_urls:
                continue
            seen_urls.add(url)
            merged.append((media_type, url, bytes(thumb)))
            if len(merged) >= limit:
                return
    ...
    if pexels_video_share > 0:
        add_items(
            VIDEO_MEDIA,
            safe_fetch(lambda: fetch_pexels_video_results(query, limit=pexels_video_share)),
        )
    ...
    if pixabay_video_share > 0:
        add_items(
            VIDEO_MEDIA,
            safe_fetch(lambda: fetch_pixabay_video_results(query, limit=pixabay_video_share)),
        )
